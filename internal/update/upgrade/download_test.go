...
func TestFindBinaryInTar(t *testing.T) {
	...
	entries := []struct {
		...
		{"tdt-ai_1.5.0_darwin_arm64/tdt-ai", content}, // binary in subdir
	}
	...
	outPath := filepath.Join(t.TempDir(), "tdt-ai")
	...
	err := downloadBinary(context.Background(), server.URL+"/release.tar.gz", "tdt-ai", outPath)
	if err != nil {
		t.Fatalf("downloadBinary: %v", err)
	}
